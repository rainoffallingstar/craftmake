...
func TestBeaverRNAStep1RunsLocallyAndUsesCache(t *testing.T) {
	...
	writeBeaverRNAStep1ProjectFixture(t, projectDirectory)

	workflowPath := filepath.Join(repositoryRoot, "workflows", "BeaverRNA", "step1.yaml")
	...
	for _, expectedOutput := range []string{
		...
		if _, err := os.Stat(filepath.Join(projectDirectory, expectedOutput)); err != nil {
			t.Fatalf("expected BeaverRNA output %q: %v", expectedOutput, err)
		}
	}
...
func writeBeaverRNAStep1ProjectFixture(t *testing.T, projectDirectory string) {
	t.Helper()
...
workflow:
  mode: RNASEQ
  jobid: beaverrna-step1-integration
  userid: integration
  species:
    primary: human
    name: human
...
reference:
  ...
  rnaseq:
    ...
    ref: [references/human.transcripts.fasta]
`
